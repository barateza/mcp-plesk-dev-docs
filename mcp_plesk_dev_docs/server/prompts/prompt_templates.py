...
1. Use `search_mcp_plesk_dev_docs(query="{extension_name} development guide",
   category="guide")` to find relevant architectural patterns.
2. Use `search_mcp_plesk_dev_docs(query="{target_language} sdk hooks",
...
1. Search for the "{api_operation}" specification using
   `search_mcp_plesk_dev_docs(query="{api_operation}", category="api")`.
...
1. Retrieve the command details using
   `search_mcp_plesk_dev_docs(query="{command_name}", category="cli")`.
